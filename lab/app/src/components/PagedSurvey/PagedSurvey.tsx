...
export interface PagedSurveyProps {
  ...
  onComplete?: (responses: Record<string, Record<string, unknown>>) => void | Promise<void>
  backLabel?: string
  nextLabel?: string
  finishLabel?: string
...
type NavigationGuard = (action: ButtonAction) => boolean | undefined | Promise<boolean | undefined>

const EMPTY_SURVEY_PROPS: Omit<SurveyProps, 'registerNavigationGuard' | 'onSubmitValues'> = {}
...
export function PagedSurvey(props: PagedSurveyProps): ReactElement | null {
  ...
  const storePageResponses = useCallback((pageId: string, values: Record<string, unknown>) => {
  ...
  const surveyProps: SurveyProps = useMemo(() => {
    const { onSubmitValues: _ignoredOnSubmit, registerNavigationGuard: _ignoredRegister, ...rest } = currentSurvey
  ...
  }, [currentSurvey, handleSubmitValues, pageInitialValues, registerNavigationGuard])

  if (!currentPage) {
    return <div className="text-sm text-slate-500">No survey pages configured.</div>
  }
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="mt-4 flex items-center justify-between gap-4 border-t border-slate-200 pt-6">
        <Button variant="ghost" onClick={handleBack} disabled={!hasPrevious || pendingAction !== null}>
          {backLabel}
        </Button>
