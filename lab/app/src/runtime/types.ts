...
type ComponentEventName<TType extends keyof ComponentEventMap> = ComponentEventMap[TType]

export type ComponentEventsConfig<TType extends string = string> = TType extends keyof ComponentEventMap
  ? Partial<Record<ComponentEventName<TType>, ComponentEventDefinition>>
  : Partial<Record<string, ComponentEventDefinition>>

export type Button = { id: string; text: string; action: ButtonAction; events?: ComponentEventsConfig<'button'> }
...
export type MediaComponent = ComponentInstance<'media', { type: string; src: string; alt?: string; label?: string; captions?: string }>
...
export type EventPayload<TType extends string = string, TData extends Record<string, unknown> = Record<string, unknown>> = {
  type: TType
  timestamp: string
