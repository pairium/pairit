...
import {
	PagedSurvey,
	type PagedSurveyPage,
	type PagedSurveyProps,
} from "./PagedSurvey";
...
export const PagedSurveyRuntime = defineRuntimeComponent<
	"paged_survey",
	Partial<PagedSurveyProps>
>({
	type: "paged_survey",
	renderer: ({ component, context }) => {
		const props = component.props as Partial<PagedSurveyProps>;
		const sessionId = context.sessionId;
		const completeAction = normalizeButtonAction(
			(props as { completeAction?: unknown }).completeAction,
		);
		...
		const pages = (props.pages ?? []).flatMap((page) => {
			...
			const pageId =
				typeof page.id === "string" && page.id.trim().length ? page.id : null;
			const surveyCandidate = (page as { survey?: unknown }).survey;
			const survey = isRecord(surveyCandidate)
				? (surveyCandidate as PagedSurveyPage["survey"])
				: null;
			...
			const originalSubmit =
				typeof typedSurvey.onSubmitValues === "function"
					? typedSurvey.onSubmitValues
					: undefined;
						...
						onSubmitValues: async (values: Record<string, unknown>) => {
							...
							if (sessionId) {
								try {
									...
									console.error(
										"Failed to submit paged survey page event",
										error,
									);
								}
							}
		...
		const handleComplete = async (
			...
			if (sessionId) {
				try {
					...
					console.error(
						"Failed to submit paged survey completion event",
						error,
					);
				}
			}
...
function normalizeButtonAction(action: unknown): ButtonAction | null {
	...
	const skipValidation =
		typeof action.skipValidation === "boolean"
