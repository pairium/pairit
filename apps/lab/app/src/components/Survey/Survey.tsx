...
export function Survey(props: SurveyProps): ReactElement | null {
	...
	if (!hasItems) {
		return (
			<div className="text-sm text-slate-500">No survey items configured.</div>
		);
	}
	...
	return (
		<div className="flex flex-col gap-8">
			{finalTitle ? (
				<h2 className="text-xl font-semibold text-slate-900">{finalTitle}</h2>
			) : null}
			{finalIntro ? (
				<p className="text-base text-slate-600">{finalIntro}</p>
			) : null}
...
function renderAnswerInput({
	item,
	field,
}: RenderAnswerInputArgs): ReactElement {
			...
			return (
				<FieldSet className="gap-3">
					...
					{answer.choices?.map((choice) => {
						...
						const inputId = `${item.id}_${choice.value}`.replace(
							/[^a-zA-Z0-9_-]/g,
							"_",
						);
						return (
...
function resolveSurveyContent({
	definition,
	items,
	source,
}: ResolveSurveyContentInput): ResolvedSurveyContent {
	...
	const rawItems = Array.isArray(items)
		...
		: Array.isArray(definition)
			? definition
			: containerCandidate &&
					Array.isArray((containerCandidate as { items?: unknown }).items)
				? ((containerCandidate as { items?: unknown[] }).items ?? [])
				: [];

	const normalizedSource =
		typeof source === "string" && source.trim().length
			? source.trim().replace(/\s+/g, "_")
			: "survey";
	...
	const resolvedTitle =
		containerCandidate &&
		typeof (containerCandidate as { title?: unknown }).title === "string"
			? ((containerCandidate as { title?: string }).title ?? "").trim()
			: undefined;

	const resolvedIntro =
		containerCandidate &&
		typeof (containerCandidate as { intro?: unknown }).intro === "string"
...
function normalizeSurveyItem(
	raw: unknown,
	index: number,
	options: { idPrefix: string },
): SurveyItemDefinition | null {
	if (!isRecord(raw)) return null;

	const record = raw as Record<string, unknown>;

	const idCandidate = typeof record.id === "string" ? record.id.trim() : "";
	const id = idCandidate.length
		? idCandidate
	...
	const description =
		typeof record.description === "string"
			...
			: undefined;
	const media =
		record.media && isRecord(record.media)
...
function normalizeSurveyAnswer(
	...
	if (typeof answerInput === "string") {
		...
		if (typeof answerRecord.type === "string") {
			...
			answerConfig =
				answerRecord.props && isRecord(answerRecord.props)
					? (answerRecord.props as Record<string, unknown>)
					: answerRecord;
	...
	if (normalizedType === "custom" || normalizedType === "component") {
		const componentId =
			typeof answerConfig.component === "string"
		...
		const props =
			answerConfig.props && isRecord(answerConfig.props)
				? (answerConfig.props as Record<string, unknown>)
				: answerConfig;
		return {
			type: "custom",
			required,
	...
	switch (normalizedType) {
		...
		case "number": {
			const min =
				typeof numericBoundsSource.min === "number"
					? numericBoundsSource.min
					: typeof record.min === "number"
						? record.min
						: undefined;
			const max =
				typeof numericBoundsSource.max === "number"
					? numericBoundsSource.max
					: typeof record.max === "number"
						? record.max
						: undefined;
			const step =
				typeof numericBoundsSource.step === "number"
					? numericBoundsSource.step
	...
	for (const entry of raw) {
		...
		if (isRecord(entry)) {
			...
			const valueCandidate =
				choiceRecord.value ?? choiceRecord.id ?? choiceRecord.slug;
	...
	switch (answer.type) {
		...
		case "multi_select": {
			...
			const sanitized = raw
				...
				.filter(
					(value) =>
						value.length && (!allowedChoices.size || allowedChoices.has(value)),
				);
...
function buildFormSchema(items: SurveyItemDefinition[]): {
	schema: SurveySchema;
	defaults: FormDefaultValues;
} {
...
function buildNumericSchema(answer: SurveyItemAnswer): z.ZodTypeAny {
	const base = z.string({ required_error: "Required" });
	let schema: z.ZodTypeAny = answer.required
