...
export const ButtonsRuntime = defineRuntimeComponent<'buttons', ButtonsComponent['props']>({
  type: 'buttons',
  renderer: ({ component, context }) => {
    ...
    return (
      <div className="flex flex-wrap gap-3">
        {buttons.map((button) => (
          <Button key={button.id} type="button" onClick={() => void handleButtonClick(button, component.id, context)}>
            {button.text}
          </Button>
        ))}
      </div>
...
async function handleButtonClick(button: ButtonDefinition, componentId: string | undefined, context: RuntimeComponentContext) {
  await emitButtonEvent(button, componentId, context)
  await Promise.resolve(context.onAction(button.action))
}

async function emitButtonEvent(button: ButtonDefinition, componentId: string | undefined, context: RuntimeComponentContext) {
