...
export const MediaRuntime = defineRuntimeComponent<
	"media",
...
function createEventEmitters(
	...
	return {
		onPlay: onPlayEmitter
			? () => {
					void onPlayEmitter();
				}
			: undefined,
		onPause: onPauseEmitter
			? () => {
					void onPauseEmitter();
				}
			: undefined,
		onSeek: onSeekEmitter
			? (currentTime) => {
					void onSeekEmitter({ current_time: currentTime });
				}
			: undefined,
		onComplete: onCompleteEmitter
			? () => {
					void onCompleteEmitter();
				}
			: undefined,
		onError: onErrorEmitter
			? (error) => {
					void onErrorEmitter({ error_message: error.toString() });
				}
			: undefined,
	};
}
